...
from model_DAN import Model
# from model_JAN import Model
...
for iii in range(3):
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            f11 = open('Top10_DAN_zhihu_AC_10_weight_200c_update.txt', 'a')
            f11.write(str(epoch) + '\t' + str(HR) + '\t' + str(NDCG) + '\t'
                      + str(recall) + '\t' + str(precision)
                      + '\t' + str(logloss) + '\t' + str(test_auc) + '\t' + str(loss_test) + '\n')
            # f11_20 = open('Top1_DAN_zhihu_AC_510_weight_200c_update.txt', 'a')
