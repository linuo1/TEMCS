...
class Model(object):
    def __init__(self,user_count, item_count, item_matrix, batch_size):
       ...
       # A可以视为books     B可以视为movies     C可以视为musics
        ...
        dis_AC = self.JAN_loss(batch_size, encoder_A_list, encoder_C_list, kernel_muls = [2.0, 2.0, 2.0], kernel_nums = [5, 2, 1],
                               fix_sigma_list = [None, 3.0, 1.68], linear=True)

        # encoder_A_list = [sec_fea_A, encoder_A]
        # encoder_C_list = [sec_fea_C, encoder_C]
        # dis_AC = self.JAN_loss(batch_size, encoder_A_list, encoder_C_list, kernel_muls=[2.0, 2.0],
        #                        kernel_nums=[5, 1],fix_sigma_list=[None, 2.0], linear=True)
    ...
    def JAN_loss(self, batch_size, source_list, target_list, kernel_muls=[2.0, 2.0], kernel_nums=[5, 1],
                 fix_sigma_list=[None, 1.68], linear=False):
        # batch_size = source_list[0].shape[0]
        ...
        if linear:
            ...
            for i in range(batch_size):
                ...
                loss += tf.squeeze(tf.slice(self.weight, [i], [1])) * (joint_kernels[s1, s2] + joint_kernels[t1, t2])
                loss -= tf.squeeze(tf.slice(self.weight, [i], [1])) * (joint_kernels[s1, t2] + joint_kernels[s2, t1])
            JAN_loss = loss / float(batch_size)
        else:
            loss1 = 0
