...
class Model(object):
    def __init__(self,user_count, item_count, item_matrix, batch_size):
       ...
       # A可以视为books     B可以视为movies     C可以视为musics
        ...
        encoder_A = self.encoder_A(item_emb_w, self.hist_A, i_emb_C, embedding_size, self.keep_prob)

        encoder_C = self.encoder_C(item_emb_w, self.hist_C, i_emb_C, embedding_size, self.keep_prob)

        # 定义两个域之间的距离 与 目标域分类器 的输出
        self.logits = self.prediction_C(i_emb_C, encoder_A, encoder_C, self.keep_prob)

        dis_AC = self.DAN_1_loss(batch_size, encoder_A, encoder_C, kernel_mul=2.0, kernel_num=1, fix_sigma=None, linear=True)
    ...
    def encoder_A(self, item_emb_w_A, seq_A, i_emb, embedding_size, keep_prob):
        with tf.variable_scope('encoder_A'):
            ...
            encoder_output_A = tf.reshape(outputs, [-1, embedding_size])

        return encoder_output_A
    ...
    def encoder_C(self, item_emb_w_C, seq_C, i_emb, embedding_size, keep_prob):
        with tf.variable_scope('encoder_C'):
            ...
            # outputs = tf.nn.dropout(outputs, keep_prob)
            encoder_output_C = tf.reshape(outputs, [-1, embedding_size])

        return encoder_output_C
