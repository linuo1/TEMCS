...
class Model(object):
    def __init__(self,user_count,item_count,item_matrix, batch_size):
        ...
        uuu = level_att(u_din, u_follow, u_vote, i_emb, embedding_size)
        inp = tf.concat([uuu, i_emb], 1)
...
def level_att(u_din, u_follow, u_vote, i_emb, embedding_size):
    ...
    queries = tf.expand_dims(i_emb, axis=1)  # B*1*H
    # queries = tf.tile(queries, multiples=[1, 3, 1])

    final = tf.concat([queries, keys], axis=1)  # [B,4, H]
    final = tf.reshape(final, [-1, 4 * embedding_size])
    print("final_shape", final.shape)  # (?, 1, 300)

    W_t = tf.Variable(
        tf.truncated_normal(shape=[4 * embedding_size, 3], stddev=0.01, dtype=tf.float32))
    b_t = tf.Variable(tf.truncated_normal(shape=[1], stddev=0.01, dtype=tf.float32))
    ###该
    score = tf.sigmoid(tf.matmul(final, W_t) + b_t)
    alpha = tf.reshape(score, [-1, 1, 3])  # score: [batch_size,time_step,1]

    # Weighted Sum
    outputs = tf.matmul(alpha, keys)  # B * 1 * H 三维矩阵相乘，相乘发生在后两维，即 B * (( 1 * T ) * ( T * H ))
    outputs = tf.reshape(outputs, [-1, embedding_size])
    return outputs
