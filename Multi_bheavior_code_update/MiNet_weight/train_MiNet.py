...
from model_multi_MiNet import Model
...
def _eval(sess, test_set, model):
    ...
    return test_auc
###明天试试所有的batch_size=25的效果
with open('../../data/zhihu_data/item_matrix_510_split.pkl', 'rb') as f:
    item_matrix = pickle.load(f)
item_matrix = item_matrix.astype(np.float32)

with open('../../data/zhihu_data/train_test_set_510_weight_200c_update.pkl', 'rb') as f2:
# with open('../data/zhihu_data/train_test_set_10.pkl', 'rb') as f2:
...
for iii in range(3):
    lr = 0.001
    # f111 = open('Computer_DIN_time.txt', 'a')
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            for _, uij in DataInput(train_set, batch_size):
                ...
                # print("trans_fol", trans_fol)
                # print("loss_fol", loss_fol)
                loss_sum += loss
                j += 1
            logloss = loss_sum / j
            ...
            f11 = open('Top10_MiNet_zhihu_510_weight_200c_update.txt', 'a')
            # f11 = open('Top10_EHCF_zhihu_BC_510.txt', 'a')
            f11.write(str(epoch) + '\t' + str(HR) + '\t' + str(NDCG) + '\t'
                      + str(recall) + '\t' + str(precision)
                      + '\t' + str(logloss) + '\t' + str(test_auc) + '\t' + str(loss_test) + '\n')
            # f11_20 = open('Top1_EHCF_zhihu_AC_510_weight_200c_update.txt', 'a')
