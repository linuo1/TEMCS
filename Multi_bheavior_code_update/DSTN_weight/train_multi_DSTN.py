...
from model_multi_DSTN import Model
...
for iii in range(4):
    ...
    with tf.Session(config=config) as sess:
        ...
        model = Model(user_count, item_count, item_matrix)
        ...
        for epoch in range(epochs):
            ...
            HR, NDCG, recall, precision, loss_test = evaluate_model(sess, model, test_set,
                                                                    10)
            # HR_20, NDCG_20, recall_20, precision_20, loss_test_20 = evaluate_model(sess, model, test_set,
            #                                                                        20)
            ...
            f11 = open('Top10_DSTN_zhihu_10_weight_200c_update.txt', 'a')
            f11.write(str(epoch) + '\t' + str(HR) + '\t' + str(NDCG) + '\t'
                      + str(recall) + '\t' + str(precision)
                      + '\t' + str(logloss) + '\t' + str(test_auc) + '\t' + str(loss_test) + '\n')

            # f11_20 = open('DIN_5_Top20m.txt', 'a')
            # f11_20.write(str(epoch) + '\t' + str(HR_20) + '\t' + str(NDCG_20) + '\t'
            #              + str(recall_20) + '\t' + str(precision_20)
            #              + '\t' + str(logloss) + '\t' + str(test_auc) + '\t' + str(loss_test_20) + '\n')
