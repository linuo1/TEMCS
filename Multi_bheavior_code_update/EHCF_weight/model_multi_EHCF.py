...
class Model(object):
    def __init__(self,user_count,item_count,item_matrix, batch_size):
        ...
        # item_emb_w = tf.get_variable('user_emb_w', [item_count, embedding_size],
        #                              initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.01))
        ...
        # follow->answer
        self.W_fa = tf.Variable(
            tf.truncated_normal(shape=[embedding_size, embedding_size], stddev=0.01, dtype=tf.float32))
        self.R_fa = tf.Variable(tf.truncated_normal(shape=[1], stddev=0.01, dtype=tf.float32))

        # vote->answer
        self.W_va = tf.Variable(
            tf.truncated_normal(shape=[embedding_size, embedding_size], stddev=0.01, dtype=tf.float32))
        self.R_va = tf.Variable(tf.truncated_normal(shape=[1], stddev=0.01, dtype=tf.float32))

        u_follow_convert = tf.matmul(u_follow, self.W_fa) + self.R_fa
        u_vote_convert = tf.matmul(u_vote, self.W_va) + self.R_va
        inp = tf.concat([u_din, u_follow_convert, u_vote_convert, i_emb], axis=-1)
        # inp = tf.concat([u_din, u_follow_convert, i_emb], axis=-1)

        dnn1 = tf.layers.dense(inp, 160,
        ...
        dnn2 = tf.layers.dense(dnn1, 1,
                               ...
                               bias_initializer=tf.constant_initializer(0.01),
                               activation=None)
        res_2 = tf.nn.dropout(dnn2, self.keep_prob)
        y_hat = tf.reshape(res_2, [-1])
        self.logits = y_hat
        ...
        #     tf.square(u_din - u_follow_convert)
        # )
        #
        # loss_2 = tf.reduce_mean(
        #     tf.square(u_din - u_vote_convert)
        # )

        # self.loss = loss_y + 0.005 * loss_1 + 0.005 * loss_2
        self.loss = loss_y
...
def get_att(hist_i, item_emb_all, embedding_size, i_emb, sl):
    ...
    # print("max_item_length", max_item_length,max_item_length*embedding_size)
    # h_emb = tf.reshape(h_emb, [-1, max_item_length * embedding_size])
